...
- `BuildArchitectImage` — builds the architect image from `packages/compile/base/architect.Dockerfile`.

## Related

- **Imported by** — `apps/api`, `apps/cli`
- **Imports** — `packages/world` (state, runtime, labels), `packages/compile` (backend, build), `packages/transpile`, `packages/runtimes`, `packages/agent`, `packages/activity`, `packages/auth`, `packages/platform`, `catalog`

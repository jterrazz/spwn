...
func TestParse_ScopeWithNoName(t *testing.T) {
	got := deps.ParseRef("@spwn/")
	if got.Kind != deps.KindSpwnBuiltin {
...
func TestParse_VersionedRefNotStripped(t *testing.T) {
	...
	got := deps.ParseRef("@spwn/unix@24.04")
	if got.Kind != deps.KindSpwnBuiltin {
		t.Errorf("kind: want KindSpwnBuiltin, got %v", got.Kind)
	}
...
func TestSplitVersion_ScopedVersioned(t *testing.T) {
	pack, version := deps.SplitVersion("@spwn/unix@24.04")
	if pack != "@spwn/unix" {
...
func TestSplitVersion_NoVersion(t *testing.T) {
	pack, version := deps.SplitVersion("@spwn/unix")
	if pack != "@spwn/unix" {
		t.Errorf("pack: want %q, got %q", "@spwn/unix", pack)
	}
	if version != "" {
...
func TestSplitVersion_GithubVersioned(t *testing.T) {
	pack, version := deps.SplitVersion("github.com/owner/repo@v1.2.3")
...
func TestResolveTool_EmptyRoot(t *testing.T) {
	...
	got := deps.ResolveTool("", deps.ParseRef("something"), nil, false)
...
func TestResolveTool_LocalNameWithSlash(t *testing.T) {
	root := t.TempDir()
	got := deps.ResolveTool(root, deps.Ref{Kind: deps.KindLocal, Name: "foo/bar"}, nil, false)
	if got != deps.ResolveNotFound {
...
func TestResolveTool_LocalNameWithDotDot(t *testing.T) {
	root := t.TempDir()
	got := deps.ResolveTool(root, deps.Ref{Kind: deps.KindLocal, Name: "../escape"}, nil, false)
	if got != deps.ResolveNotFound {
...
func TestResolveTool_BuiltinWithoutCatalog(t *testing.T) {
	...
	got := deps.ResolveTool("", deps.ParseRef("@spwn/anything"), nil, false)
...
func TestResolveTool_BuiltinWithCatalogMissing(t *testing.T) {
	...
	got := deps.ResolveTool("", deps.ParseRef("@spwn/not-in-catalog"), catalog, true)
	if got != deps.ResolveNotFound {
...
func TestResolveSkill_MdPathIsDirectory(t *testing.T) {
	...
	got := deps.ResolveSkill(root, deps.ParseRef("trick"), nil, false)
...
func TestResolveSkill_EmptyPackDir(t *testing.T) {
	...
	got := deps.ResolveSkill(root, deps.ParseRef("empty-pack"), nil, false)
...
func TestResolveSkill_RegistryAlwaysUnsupported(t *testing.T) {
	got := deps.ResolveSkill("", deps.ParseRef("@acme/foo"), nil, false)

...
func NewTestContext(t *testing.T) *TestContext {
	...
	store, err := universe.NewStoreAt(filepath.Join(baseDir, "state.json"))
	if err != nil {
	...
	ctx := &TestContext{
		...
		Backend: docker,
		State:   store,
		Arc:     universe.NewArchitect(docker, store),
	}
...
// TestdataDir returns the absolute path to fixtures/testdata/ in the repo.
func TestdataDir() string {
	dir, _ := os.Getwd()
	for {
		if _, err := os.Stat(filepath.Join(dir, "go.work")); err == nil {
			return filepath.Join(dir, "fixtures", "testdata")
		}
	...
	return filepath.Join("fixtures", "testdata")
}

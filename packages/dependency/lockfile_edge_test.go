...
func TestLoad_githubStyleRef(t *testing.T) {
	...
	if e.Source != dependency.SourceGitHub {
		t.Errorf("source = %q, want %q", e.Source, dependency.SourceGitHub)
	}
}
...
func TestRoundtrip_githubRefs(t *testing.T) {
	...
	l.Add("github.com/jterrazz/research-skills", dependency.LockEntry{
		Version: "v0.3.0",
	...
	if e.Version != "v0.3.0" || e.Source != dependency.SourceGitHub {
...
func TestAdd_lastWins(t *testing.T) {
	...
	if e.Source != dependency.SourceGitHub {
		t.Errorf("source = %q, want %q (last wins)", e.Source, dependency.SourceGitHub)
	}
}
...
func TestLoad_veryLongRefAndVersion(t *testing.T) {
	...
	l := dependency.EmptyLockfile()
	l.Add(longRef, dependency.LockEntry{Version: longVersion, Source: dependency.SourceGitHub})

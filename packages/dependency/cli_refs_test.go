...
func TestResolveCLI(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := dependency.ResolveCLI(tc.input, catalog)
			if tc.errSub == "" {
				if err != nil {
...
func TestResolveCLI_NoAliasing(t *testing.T) {
	catalog := []string{"qmd"}
	input := "qmd@latest"
	got, err := dependency.ResolveCLI(input, catalog)
	if err != nil {
		t.Fatal(err)
	}

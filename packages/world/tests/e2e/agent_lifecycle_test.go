...
func TestAgentLifecycle_SessionDiffersPerWorld(t *testing.T) {
	...
	sessionA := agentDomain.DeterministicSessionID("session-diff-agent", chainA.World().ID)
	sessionB := agentDomain.DeterministicSessionID("session-diff-agent", chainB.World().ID)

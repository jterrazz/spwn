...
import { apiGet, apiPost, apiAction } from "@/lib/api-client";
...
type StepId = "docker" | "auth" | "world" | "chat";
...
export default function WelcomePage() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      setActive((prev) => {
        if (prev !== "docker" && completed(prev, s)) return nextStep(prev);
        if (!s.hasDocker) return "docker";
        if (!s.hasAuth) return "auth";
        if (!s.hasWorlds) return "world";
        return "chat";
      });
    } catch {
      ...
      <div className="mx-auto max-w-3xl">
        <header className="mb-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-lg text-sm text-muted-foreground/70">
            The control plane for AI agents. Four quick steps and you&apos;re ready to spawn your first world.
          </p>
        </header>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-sm font-medium ${
                          done ? "text-emerald-200/90" : "text-foreground/90"
                        }`}
                      >
                        {step.title}
                      </span>
...
function StepBody({
  ...
  if (stepId === "docker") return <DockerStep status={status} onRefresh={onRefresh} refreshing={refreshing} onAdvance={onAdvance} />;
  if (stepId === "auth") return <AuthStep status={status} onAdvance={onAdvance} />;
  if (stepId === "world") return <WorldStep status={status} onAdvance={onAdvance} />;
  return <ChatStep />;
}
...
function AuthStep({ status, onAdvance }: { status: OnboardingStatus | null; onAdvance: () => void }) {
...
function WorldStep({ status, onAdvance }: { status: OnboardingStatus | null; onAdvance: () => void }) {
  return (
    <div className="space-y-3">
      <p className="text-[11px] text-muted-foreground/60">
        Open the worlds page and click <span className="text-foreground/80">Spawn world</span>.
        We&apos;ll detect it as soon as a world is created.
      </p>
      <div className="flex items-center gap-2">
        <Link
          href="/"
...
function ChatStep() {
  return (
    <div className="space-y-3">
      <p className="text-[11px] text-muted-foreground/60">
        Open any world from the sidebar and send a message in the chat panel.
        Once you finish here, click <span className="text-foreground/80">Finish setup</span> below.
      </p>
      <Link
        href="/"
        className="inline-flex items-center gap-1.5 rounded-md border border-white/15 bg-white/[0.04] px-3 py-1.5 text-[11px] font-medium text-foreground/90 hover:bg-white/[0.08]"
...
function completed(step: StepId, status: OnboardingStatus): boolean {
  switch (step) {
    ...
    case "chat":
      return false; // user explicitly finishes
  }
}

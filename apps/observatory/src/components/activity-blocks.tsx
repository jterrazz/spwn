...
function ToolUseBlockView({ block, result }: { block: { tool: string; input: Record<string, unknown>; id: string }; result?: { content: string; isError: boolean } }) {
...
function ErrorBlockView({ block }: { block: { content: string } }) {
  return (
    <div className="flex items-start gap-2 rounded-lg border border-red-500/15 bg-red-500/[0.06] px-3 py-2">
      <IconAlertTriangle size={14} className="text-red-400/60 shrink-0 mt-0.5" />
...
function StatusBlockView({ block }: { block: { status: string; tool?: string } }) {
  ...
  return (
    <div className="flex items-center gap-2 py-1">
      ...
      <span className="text-[11px] text-muted-foreground/40">{labels[block.status] || block.status}</span>
    </div>
  );
}
    ...
    <div className="space-y-2">
      ...
      {message.cost !== undefined && (
        <div className="flex items-center gap-3 text-[9px] font-mono text-muted-foreground/20 pt-1">
          {message.duration !== undefined && <span>{(message.duration / 1000).toFixed(1)}s</span>}

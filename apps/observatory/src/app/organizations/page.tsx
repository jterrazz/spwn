...
const LEVEL_COLORS: Record<number, { border: string; text: string; bg: string; glow: string }> = {
  0: { border: "border-amber-500/30", text: "text-amber-300", bg: "bg-amber-500/10", glow: "shadow-[0_0_20px_rgba(245,158,11,0.08)]" },
  1: { border: "border-purple-500/25", text: "text-purple-300", bg: "bg-purple-500/10", glow: "shadow-[0_0_16px_rgba(168,85,247,0.06)]" },
  2: { border: "border-blue-500/20", text: "text-blue-300", bg: "bg-blue-500/10", glow: "" },
};
function roleColor(level: number) {
  return LEVEL_COLORS[level] ?? { border: "border-white/[0.1]", text: "text-foreground/60", bg: "bg-white/[0.04]", glow: "" };
}
...
function RoleNodeComponent({ data }: NodeProps<Node<RoleNodeData>>) {
  ...
  return (
    <div className={`border ${c.border} bg-[#0a0a0c]/90 backdrop-blur-sm rounded-xl px-6 py-4 min-w-[220px] text-center ${c.glow} transition-shadow hover:border-opacity-60`}>
      {/* Handles for edges */}
      {!isFirst && <Handle type="target" position={Position.Top} className="!bg-white/[0.15] !border-0 !w-1.5 !h-1.5" />}
      {!isLast && <Handle type="source" position={Position.Bottom} className="!bg-white/[0.15] !border-0 !w-1.5 !h-1.5" />}
      ...
      <div className="flex items-center justify-center gap-1.5 mt-2">
        <span className={`px-2 py-0.5 text-[9px] font-mono rounded-full ${c.bg} ${c.text} border ${c.border}`}>
          Level {role.level}
        </span>
      ...
      {role.permissions && role.permissions.length > 0 && (
        <div className="flex flex-wrap items-center justify-center gap-1 mt-3">
          {role.permissions.map((p) => (
            <span key={p} className="px-1.5 py-0.5 text-[8px] font-mono bg-white/[0.03] border border-white/[0.05] rounded text-muted-foreground/40">
              {p}
            </span>
          ))}
        </div>
        ...
        <div className="mt-2.5 space-y-0.5">
          {role.reports_to && (
            <p className="text-[8px] font-mono text-muted-foreground/25">
              reports to <span className="text-muted-foreground/40">{role.reports_to}</span>
            </p>
          )}
          {role.can_command && role.can_command.length > 0 && (
            <p className="text-[8px] font-mono text-muted-foreground/25">
              commands <span className="text-muted-foreground/40">{role.can_command.join(", ")}</span>
            </p>
          )}
        </div>
      )}
...
function CreateOrganizationDialog({ onClose, onComplete }: { onClose: () => void; onComplete: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    if (roles.length === 0 || roles.every((r) => !r.name.trim())) {
      setError("At least one role with a name is required"); return;
    }
    setCreating(true);
    setError("");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-lg mx-4 rounded-2xl bg-popover/95 backdrop-blur-md border border-white/[0.08] shadow-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="px-6 pb-6 space-y-4 overflow-y-auto min-h-0">
          <div>
            <label className="text-[10px] uppercase tracking-widest text-muted-foreground/40 block mb-1.5">Name <span className="text-red-400/60">*</span></label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Military Chain"
          ...
          <div>
            <label className="text-[10px] uppercase tracking-widest text-muted-foreground/40 block mb-1.5">Description</label>
            <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="A strict top-down command structure"
              className="w-full bg-white/[0.03] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-foreground/80 placeholder:text-muted-foreground/25 focus:outline-none focus:border-white/[0.15] transition-colors" />
          </div>

          <div>
            <label className="text-[10px] uppercase tracking-widest text-muted-foreground/40 block mb-2">Roles</label>
            <div className="space-y-3">
              {roles.map((role, idx) => (
                <div key={idx} className="border border-white/[0.06] rounded-lg p-3 bg-white/[0.01] space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] font-mono text-muted-foreground/30">#{idx + 1}</span>
                    <div className="flex-1" />
                    {roles.length > 1 && (
                      <button onClick={() => removeRole(idx)} className="text-muted-foreground/30 hover:text-red-400/70 transition-colors">
                        <IconX size={14} />
                      </button>
                    )}
                  </div>
                  <div className="grid grid-cols-[1fr_60px] gap-2">
                    <input value={role.name} onChange={(e) => updateRole(idx, { name: e.target.value })} placeholder="Role name"
                    ...
                    <input type="number" value={role.level} onChange={(e) => updateRole(idx, { level: parseInt(e.target.value) || 0 })} placeholder="Lvl"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <select value={role.reports_to} onChange={(e) => updateRole(idx, { reports_to: e.target.value })}
                        ...
                        {roleNames.filter((n) => n !== role.name.trim()).map((n) => <option key={n} value={n}>{n}</option>)}
                      </select>
                    </div>
                    <div>
                      <span className="text-[8px] font-mono text-muted-foreground/25 uppercase block mb-0.5">Can command</span>
                      <input value={role.can_command} onChange={(e) => updateRole(idx, { can_command: e.target.value })} placeholder="role1, role2"
                  ...
                  <div>
                    ...
                    <input value={role.permissions} onChange={(e) => updateRole(idx, { permissions: e.target.value })} placeholder="delegate, review, execute"
                      className="w-full bg-white/[0.03] border border-white/[0.08] rounded px-2 py-1.5 text-xs font-mono text-foreground/80 placeholder:text-muted-foreground/25 focus:outline-none focus:border-white/[0.15] transition-colors" />
                  </div>
                </div>
              ))}
            </div>
            <button onClick={() => setRoles((prev) => [...prev, emptyRoleDraft()])}
          ...
          <div className="flex gap-3 justify-end pt-2">
            <button onClick={onClose} disabled={creating}
              className="px-4 py-2 rounded-lg text-sm text-muted-foreground/60 hover:text-foreground/80 hover:bg-white/[0.04] transition-colors disabled:opacity-50">Cancel</button>
            <button onClick={handleCreate} disabled={creating}
              ...
              {creating ? <><div className="w-3 h-3 border-2 border-emerald-300/40 border-t-emerald-300 rounded-full animate-spin" />Creating...</> : "Create"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-sm mx-4 rounded-2xl bg-popover/95 backdrop-blur-md border border-white/[0.08] shadow-2xl p-6">
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button onClick={handleDelete} disabled={deleting}
            ...
            {deleting ? <><div className="w-3 h-3 border-2 border-red-300/40 border-t-red-300 rounded-full animate-spin" />Deleting...</> : <><IconTrash size={14} />Delete</>}
          </button>
        </div>
      </div>
    </div>
            ...
            <div key={h.slug}>
              ...
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-52 shrink-0 space-y-2">
                  ...
                  <KeyValue label="Max Depth" value={Math.max(...h.roles.map((r) => r.level))} />
                </div>
              </div>

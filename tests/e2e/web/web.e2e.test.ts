...
describe('web - spwn web', () => {
    ...
    test("'spwn web' should bind to a port and serve HTTP", async () => {
        ...
        try {
            ...
            while (Date.now() - startTime < 10_000) {
                ...
                const portMatch = combined.match(/(?:localhost|127\.0\.0\.1|0\.0\.0\.0):(\d{4,5})/);
                if (portMatch) {
                    port = portMatch[1];
                    break;
                }
    ...
    test("'spwn web' should clean up on SIGTERM", async () => {
        ...
        const startTime = Date.now();
        // eslint-disable-next-line no-unmodified-loop-condition -- `exited` is mutated from the exit event handler

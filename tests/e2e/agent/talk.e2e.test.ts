...
describe('agent talk', () => {
    ...
    test('talk can be called multiple times on same world', () => {
        ...
        // WHEN - talking twice
        const talk1 = ctx.spwn(['agent', 'talk', 'neo', 'hello'], 60_000);
        const talk2 = ctx.spwn(['agent', 'talk', 'neo', 'hello again'], 60_000);
    ...
    test('ls shows agent names', () => {
        ...
        ctx = createTestContext();
        ctx.spwn(['init']);
        const _spawnResult = ctx.spwn(['world', '--agent', 'neo', '-w', ctx.home], 60_000);
    ...
    test('talk skips dead containers and finds the live one', () => {
        ...
        const talkResult = ctx.spwn(['agent', 'talk', 'neo', 'hello'], 60_000);

        // THEN - connects to the live world (id2), not the dead one (id1)
        expect(talkResult.exitCode).toBe(0);
        expectLine(
            talkResult.output,
            new RegExp(id2.replace(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`)),
        );
    });

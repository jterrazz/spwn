...
describe('agent evolution', () => {
    ...
    test('dream with no journal skips', async () => {
        ...
        const result = await spwn('dream empty').exec('agent dream neo').run();
    ...
    test('sleep on fresh agent - nothing to archive', async () => {
        ...
        const result = await spwn('sleep fresh').exec('agent sleep neo').run();
    ...
    test('fork creates new agent', async () => {
        ...
        const result = await spwn('fork agent').exec('agent fork neo neo-v2').run();
        ...
        expectLine(result.output, /✓ Layers copied\s+core, skills, knowledge, playbooks, journal/);
    });
    ...
    test('fork duplicate target fails', async () => {
        ...
        const result = await spwn('duplicate fork').exec('agent fork neo neo-v2').run();
    ...
    test('fork preserves source Mind layers', async () => {
        // WHEN - forking an agent
        const forkResult = await spwn('fork for show').exec('agent fork neo neo-clone').run();
        expect(forkResult.exitCode).toBe(0);

        // THEN - the forked agent has the same Mind structure
        const inspectResult = await spwn('show forked').exec('agent show neo-clone').run();
    ...
    test('sleep on non-existent agent is a no-op', async () => {
        ...
        const result = await spwn('sleep missing').exec('agent sleep nonexistent').run();
    ...
    test('dream with journal entries creates auto-reflexion.md', async () => {
        ...
        // WHEN - dreaming
        const result = await spwn('dream with journal').exec('agent dream neo').run();
    ...
    test('dream output includes success rate', async () => {
        ...
        // WHEN - dreaming
        const result = await spwn('dream success rate').exec('agent dream neo').run();
    ...
    test('sleep archives stale playbooks', async () => {
        ...
        // WHEN - sleeping
        const result = await spwn('sleep stale').exec('agent sleep neo').run();
    ...
    test('sleep preserves fresh files', async () => {
        ...
        // WHEN - sleeping
        const result = await spwn('sleep fresh files').exec('agent sleep neo').run();
    ...
    test('dream auto-reflexion.md content references journal entries', async () => {
        ...
        // WHEN - dreaming
        const result = await spwn('dream content check').exec('agent dream neo').run();

        // THEN - auto-reflexion.md is created
        expect(result.exitCode).toBe(0);
        const reflexionPath = join(home, 'agents', 'neo', 'playbooks', 'auto-reflexion.md');
        expect(existsSync(reflexionPath)).toBe(true);

        // AND - its content is non-empty and structured
        const content = readFileSync(reflexionPath, 'utf8');
    ...
    test('dream is idempotent - running twice overwrites cleanly', async () => {
        ...
        const result = await spwn('dream second').exec('agent dream neo').run();
        ...
        const playbooksDir = join(home, 'agents', 'neo', 'playbooks');
        const reflexionFiles = readdirSync(playbooksDir).filter((f) =>
            f.includes('auto-reflexion'),
        );
    ...
    test('sleep archives stale but keeps fresh in same directory', async () => {
        ...
        // WHEN - sleeping
        const result = await spwn('sleep mixed').exec('agent sleep neo').run();
    ...
    test('sleep archives stale knowledge files', async () => {
        ...
        const knowledgeDir = join(home, 'agents', 'neo', 'knowledge');
        ...
        // WHEN - sleeping
        const result = await spwn('sleep knowledge').exec('agent sleep neo').run();
    ...
    test('sleep prunes old sessions', async () => {
        ...
        // WHEN - sleeping
        const result = await spwn('sleep prune sessions').exec('agent sleep neo').run();
    ...
    test('dream then sleep preserves auto-reflexion.md (it is fresh)', async () => {
        ...
        const reflectResult = await spwn('dream before sleep').exec('agent dream neo').run();
        ...
        const sleepResult = await spwn('sleep after dream').exec('agent sleep neo').run();

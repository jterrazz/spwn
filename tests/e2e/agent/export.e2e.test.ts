...
describe('agent export', () => {
    ...
    test('export creates tar.gz', async () => {
        ...
        const result = await spwn('export agent').exec('agent export neo').run();
    ...
    test('export tar.gz contains core/profile.md', async () => {
        // WHEN - exporting the agent
        const result = await spwn('export contents').exec('agent export neo').run();
        expect(result.exitCode).toBe(0);
    ...
    test('export includes all Mind layers by default', async () => {
        ...
        const result = await spwn('export full').exec('agent export neo').run();
    ...
    test('import restores agent from export', async () => {
        ...
        const exportResult = await spwn('export for import').exec('agent export neo').run();
        ...
        if (importResult.exitCode === 0) {
            ...
            expect(existsSync(join(home, 'agents', 'neo-restored', 'identity', 'default.md'))).toBe(
                true,
            );
        } else {

...
describe('CLI input validation', () => {
    ...
    test("'spwn down' with no world ID shows error", async () => {
        const result = await spwn('down no id').exec('down').run();
    ...
    test("'spwn world inspect' with no world ID shows error", async () => {
        const result = await spwn('inspect no id').exec('world inspect').run();
    ...
    test("'spwn world logs' with no world ID shows error", async () => {
        const result = await spwn('logs no id').exec('world logs').run();
    ...
    test("'spwn profile' with no subcommand shows help", async () => {
        const result = await spwn('profile no args').exec('profile').run();
    ...
    test("'spwn agent send' with missing args shows error", async () => {
        const result = await spwn('agent send no args').exec('agent send').run();
    ...
    test('error messages do NOT dump full usage/help', async () => {
        ...
        for (const cmd of commands) {
            const result = await spwn(`validation: ${cmd}`).exec(cmd).run();
    ...
    test('error messages contain actionable hints', async () => {
        ...
        const result = await spwn('actionable hint').exec('down w-nonexistent-00000').run();
    ...
    test('unknown top-level command shows error without full usage dump', async () => {
        const result = await spwn('unknown command').exec('foobar').run();
    ...
    test('agent rm with no name shows error', async () => {
        const result = await spwn('agent rm no name').exec('agent rm').run();

...
describe('CLI - auth command', () => {
    ...
    test("'spwn auth token --help' shows usage", async () => {
        ...
        const result = await spwn('auth token help').exec('auth token --help').run();
    ...
    test("'spwn auth login' handles non-interactive gracefully", async () => {
        ...
        const result = await spwn('auth login noninteractive').exec('auth login').run();
    ...
    test("'spwn auth logout' is idempotent", async () => {
        ...
        const _result1 = await spwn('auth logout 1').exec('auth logout').run();

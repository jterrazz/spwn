...
describe('architect Stack flow', () => {
    ...
    test('architect status includes Stack KPIs with correct counts', () => {
        ...
        const content = readFileSync(join(architectDir, 'stack.md'), 'utf8');
    ...
    test('architect talk response includes stackAction when present', () => {
        ...
        const todoAddMatch = response.match(/\[STACK_PUSH\]\s*(.*?)(?:\n|$)/);
    ...
    test('Stack panel in the web UI is read-only (no Add Stack input)', () => {
        ...
        const source = readFileSync(projectPagePath, 'utf8');
        ...
        expect(source).not.toMatch(/handleAddTodo|handleCreateTodo|onAddTodo/); // No add handlers
    });
    ...
    test('architect Stack_DONE response parsing extracts completion info', () => {
        ...
        const todoDoneMatch = response.match(/\[STACK_POP\]\s*(.*?)(?:\n|$)/);
        ...
        expect(completedMatch![1].trim()).toBe('deployed pipeline to GitHub Actions');
    });
    ...
    test('architect Stack_UPDATE response parsing extracts progress info', () => {
        ...
        const todoUpdateMatch = response.match(/\[STACK_UPDATE\]\s*(.*?)(?:\n|$)/);
        const progressMatch = response.match(/Progress:\s*(.*?)(?:\n|$)/);

        expect(todoUpdateMatch).not.toBeNull();
        expect(todoUpdateMatch![1].trim()).toBe('Deploy API');
        expect(progressMatch).not.toBeNull();
        expect(progressMatch![1].trim()).toBe('75% complete, testing in staging');
    });
    ...
    test('Stack counting with empty sections returns zero', () => {
        ...
        const stackContent = '# Architect Stack\n\n## Focus\n\n## Queued\n\n## Done\n';
    ...
    test('Stack file write and re-read roundtrip preserves content', () => {
        ...
        // Read back
        const result = readFileSync(stackPath, 'utf8');
    ...
    test('multiple Stack actions in response - only first is recognized', () => {
        ...
        const firstMatch = response.match(/\[STACK_PUSH\]\s*(.*?)(?:\n|$)/);

...
describe('agent messaging', () => {
    ...
    test('send creates message in agent inbox', () => {
        ...
        const _id = parseWorldId(spawn.output)!;
    ...
    test('inbox shows messages for an agent', () => {
        ...
        ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'implement webhooks']);
    ...
    test('inbox shows all messages when no agent specified', () => {
        ...
        ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'task 1']);
        ctx.spwn(['agent', 'send', 'morpheus', '--from', 'neo', 'reply']);
    ...
    test('message file exists inside container', () => {
        ...
        ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'check persistence']);
    ...
    test('multiple messages to same agent all appear', () => {
        ...
        ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'first message']);
        ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'second message']);
        ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'third message']);
    ...
    test('inbox shows table with correct columns', () => {
        ...
        ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'table test']);
    ...
    test('send to non-existent agent fails', () => {
        ctx = createTestContext();
        ctx.spwn(['init']);
        const result = ctx.spwn(['agent', 'send', 'nonexistent', '--from', 'morpheus', 'hello']);
    ...
    test('send without --from defaults to user', () => {
        ...
        const result = ctx.spwn(['agent', 'send', 'neo', 'missing from']);
        expect(result.exitCode).toBe(0);

...
describe('error recovery - state resilience', () => {
    ...
    test('agent commands work after failed agent operations', async () => {
        ...
        const errorResult = await spwn('rm ghost').exec('agent rm ghost').run();
        ...
        const listResult = await spwn('list after error').exec('agent ls').run();
    ...
    test('export non-existent agent does not corrupt state', async () => {
        ...
        const listResult = await spwn('list after bad export').exec('agent ls').run();
        expect(listResult.exitCode).toBe(0);
    ...
    test('multiple errors in sequence do not compound', async () => {
        ...
        for (let i = 0; i < 3; i++) {
            const result = await spwn(`error-${i}`).exec('agent rm nonexistent').run();
    ...
    test('init is idempotent - running init twice does not break state', async () => {
        ...
        const _second = await spwn('init 2').exec('init').run();

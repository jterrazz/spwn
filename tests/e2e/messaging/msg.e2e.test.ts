...
describe('messaging - spwn agent send/inbox', () => {
    ...
    test("'spwn agent send' sends a message to a running agent", () => {
        ...
        ctx.spwn(['init']);
        const spawn = ctx.spwn(['world', 'up', '--agent', 'neo', '-w', ctx.home], 60_000);
        const id = parseWorldId(spawn.output)!;
        expect(id).toBeTruthy();

        // WHEN - sending a message
        const result = ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'hello world']);
    ...
    test("'spwn agent inbox' shows messages for an agent", () => {
        ...
        ctx.spwn(['agent', 'send', 'neo', '--from', 'morpheus', 'inbox test']);

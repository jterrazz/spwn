...
describe('state management', () => {
    ...
    test('world state persists in state.json', () => {
        ...
        ctx.state().exists().hasWorld(id).hasAgent(id, 'neo').worldCount(1);
    ...
    test('state tracks active worlds across list calls', () => {
        ...
        const idPattern = new RegExp(id.replace(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`));
    ...
    test('multiple worlds tracked in state', () => {
        ...
        // Spawn two worlds
        const r1 = ctx.spwn(['world', '--agent', 'neo', '-w', ctx.home], 60_000);
        const id1 = parseWorldId(r1.output)!;
        expectLine(r1.output, /✓ Created container\s+(?:spwn-world|w)-\w+-\d{5}/);

        const r2 = ctx.spwn(['world', '--agent', 'neo', '-w', ctx.home], 60_000);
        const id2 = parseWorldId(r2.output)!;
        expectLine(r2.output, /✓ Created container\s+(?:spwn-world|w)-\w+-\d{5}/);

        // THEN - both are tracked
        ctx.state().worldCount(2).hasWorld(id1).hasWorld(id2);

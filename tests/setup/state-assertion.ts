...
export class StateAssertion {
    ...
    private worldStatusOf(worldId: string): null | string {
        try {
            const out = execSync(`docker inspect --format='{{.State.Status}}' ${worldId}`, {
                encoding: 'utf8',
                timeout: 5000,
    ...
    private worldLabel(worldId: string, label: string): null | string {
        try {
            const out = execSync(

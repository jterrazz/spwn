...
class AgentProbeAssertion {
    private probe: Record<string, unknown>;

    constructor(probe: Record<string, unknown>) {
        this.probe = probe;
    }
...
export class WorldAssertion {
    private containerId: null | string;
    private worldId: string;

    constructor(worldId: string, _spwnHome: string) {
        ...
        try {
            const id = execSync(`docker inspect --format='{{.Id}}' ${worldId}`, {
                encoding: 'utf8',
                timeout: 5000,
    ...
    toBeRunning(): this {
        ...
        const result = execSync(`docker inspect --format='{{.State.Running}}' ${cid}`, {
            encoding: 'utf8',
            timeout: 5000,
        }).trim();
    ...
    toNotExist(): this {
        ...
        try {
            ...
            if (error instanceof Error && error.message === 'Container still exists') {
                throw error;
            }
    ...
    agentProbe(): AgentProbeAssertion {
        const cid = this.requireContainerId();
        try {
            const raw = execSync(`docker exec ${cid} cat /tmp/claude-mock.json`, {
                encoding: 'utf8',
                timeout: 5000,
            });

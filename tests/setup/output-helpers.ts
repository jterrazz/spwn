...
export function stripAnsi(str: string): string {
    // eslint-disable-next-line no-control-regex -- ANSI escape sequences are control chars
 ...
 */
export function expectTableHeader(output: string, columns: string[]): void {
    const outputLines = lines(output);
    const headerLine = outputLines.find((l) => columns.every((c) => l.includes(c)));
    if (!headerLine) {
        throw new Error(

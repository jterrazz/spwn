...
- **The repository root is a package now.** That is what Turborepo needs to find the package manager, and it also makes `tests/` a root-level test directory in the eyes of `@jterrazz/typescript`'s I2 rule — 63 diagnostics that were always true and never visible. They are recorded in `tests/oxlint.baseline.json` as debt, not fixed here.

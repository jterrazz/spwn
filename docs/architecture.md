...
- **Everything is a package**. Tools, plugins, and skills are one concept — the only thing that differs is which fields the manifest populates. A package.yaml with an `install:` block is a tool; one with a `plugin:` block injects runtime config; a bare `.md` file is a skill.

...
- **Dependencies resolve like npm**. `@spwn/<name>` is a catalog plugin compiled into the binary; `<bare-name>` is a local plugin under `spwn/plugins/<name>/` (directory form) or `spwn/plugins/<name>.md` (bare-markdown skill); `@<owner>/<name>` is reserved for a future community registry. Catalog pins live in `spwn.lock` at the project root, managed by `spwn plugin install` / `spwn plugin uninstall`. `spwn check` flags drift between agent.yaml and the lockfile.

...
type rawManifest struct {
	...
	Gate []gate.GateBridge `yaml:"gate"`
}

// Load reads a named universe config from ~/.spwn/universes/{name}.yaml.
func Load(name string) (models.UniverseManifest, error) {
	path := filepath.Join(config.UniversesDir(), name+".yaml")
	return LoadPath(path)
}

// LoadPath reads a universe config from an explicit file path.
func LoadPath(path string) (models.UniverseManifest, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return models.UniverseManifest{}, fmt.Errorf("read config %s: %w", path, err)
	}

	var raw rawManifest
	if err := yaml.Unmarshal(data, &raw); err != nil {
		return models.UniverseManifest{}, fmt.Errorf("parse config %s: %w", path, err)
	}

	m := models.UniverseManifest{
...
func ApplyDefaults(m *models.UniverseManifest) {
...
func Validate(m models.UniverseManifest) error {

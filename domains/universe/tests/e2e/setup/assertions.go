...
func (a *AssertionChain) Universe() *universe.Universe {
	return a.universe
}
...
func (s *StateAssertion) UniverseStatus(expected universe.UniverseStatus) {
	s.tc.T.Helper()
	universes := s.tc.LoadState()
	if len(universes) == 0 {
...
func (e *ListEntryAssertion) StatusIs(status universe.UniverseStatus) {
	e.tc.T.Helper()
